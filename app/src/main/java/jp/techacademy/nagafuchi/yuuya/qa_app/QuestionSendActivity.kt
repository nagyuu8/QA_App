...
class QuestionSendActivity : AppCompatActivity(), View.OnClickListener,DatabaseReference.CompletionListener {
    ...
    override fun onClick(v: View) {
        if(v === imageView){
            ...
            im.hideSoftInputFromWindow(v.getWindowToken(),InputMethodManager.HIDE_NOT_ALWAYS)//v.windowTokenじゃだめ？
            ...
            if(title.isEmpty()){
                //タイトルが入力されてないときはエラーを表示させ、終了する
                Snackbar.make(v,"タイトルを入力してください。",Snackbar.LENGTH_LONG).show()
                return
            }
            if(body.isEmpty()){
                //質問が入力されていないときはエラーを表示させ、終了（Return）させる
    ...
    private fun showChooser(){
        ...
        //EXTRA_INITAL_INTENTSにカメラ撮影のIntentを追加
